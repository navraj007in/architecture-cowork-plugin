...
**Step 4j.0 — Check for pre-calculated complexity scores:**

Before running the 10-factor scoring, check if `sdl/complexity.sdl.yaml` exists (generated by `/architect:import` or sdl-discovery).

**If `sdl/complexity.sdl.yaml` exists:**

Read it and display both indices prominently at the top of the output:

```markdown
## Architecture Complexity Index (ACI): {X.X}/10
*{Structural | Dynamic | Integration | Technology dimensions}*
{One sentence: "This system is hard to understand because..."}

## Delivery Burden Index (DBI): {X.X}/10
*{Operational | Organizational dimensions}*
{One sentence: "This system is hard to operate because..."}

## Unified Score: {X.X}/10 — {Simple | Moderate | Advanced | Very Advanced}
*(ACI × 0.6 + DBI × 0.4)*
```

Then also run the full 10-factor scoring below for cross-reference and detailed build-path guidance. Label the 10-factor section as "**Build Complexity Breakdown (10-Factor)**" to distinguish it from the ACI/DBI indices.

Note any significant discrepancy (>2 points) between the unified SDL score and the 10-factor score, and explain why they differ.

**If `sdl/complexity.sdl.yaml` does not exist:**

Proceed with 10-factor scoring only (current behaviour).
