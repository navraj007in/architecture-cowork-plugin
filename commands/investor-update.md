...
2. **SDL file** — **only if `_state.json` is absent or lacks `project`/`tech_stack`**; check `solution.sdl.yaml` first; if absent, read `sdl/README.md` then the relevant module files. Read only the `solution` and `architecture.projects` sections (Grep for these) to get project name, description, stage, and component list.
